...
{
        //added by nemo: currently we cannot deal with gzip encoding, mask it.
...
process_server_headers(struct conn_s *connptr)
{
	...
	remove_connection_headers(hashofheaders);
        ...
        /* by nemo: determine whether we need to convert charset
         */
        len = hashmap_entry_by_key(hashofheaders, "content-type", (void **)&data);
        if (len > 0) {
            char *p;
            p=strstr(data, "text/html");
            if (p!=NULL) {
                p=strstr(data, "UTF-8");
                if (p==NULL) {
                    connptr->charset=1;
                    hashmap_remove(hashofheaders,"content-type");
                    hashmap_insert(hashofheaders,"content-type","text/html;charset=UTF-8",strlen("text/html;charset=UTF-8"));
...
relay_connection(struct conn_s *connptr)
{
	...
	for (;;) {
		...
		if (FD_ISSET(connptr->server_fd, &rset)) {
                    // by nemo: change charset
                    if (connptr->charset==1) {
                        bytesin = read(connptr->server_fd, buffer, READ_BUFFER_SIZE);
                        if (bytesin > 0) {
                            if (i==1) {
                                static char *skipheaders[] = {
                                        "; charset=gb2312",
                                ...
                                for (j = 0; j != (sizeof(skipheaders) / sizeof(char *)); j++) {
                                    i=removechar(buffer,bytesin, skipheaders[j], strlen(skipheaders[j]));
                                    //i=0;
                                    if (i==0) {
                                        //bytesin=strlen(buffer);
                                        break;
                                    }
                                }
                            }
                            //by Nemo: fix "open in new window" hyperlink
                            static char *skiplink[] = {
                                        "target=\"_blank\"",
                                        "target=\'_blank\'",
                                        "target=_blank",
                                        "target= _blank"
                                };
                                for (j = 0; j != (sizeof(skiplink) / sizeof(char *)); j++) {
                                    removechar(buffer,bytesin, skiplink[j], strlen(skiplink[j]));
                                };

                            //if(0) {
                            if (i==0) {
                            iconvchar=safemalloc(bytesin * 2);
                            iconvchar[0]='\0';
                            iconv_string(cd,buffer,bytesin,iconvchar);

                            bytesout=strlen(iconvchar);
                            TRACE("%d\n",bytesout);
                            TRACE("%s\n",iconvchar);
                            } else {
                                iconvchar=buffer;
                            ...
                            if (add_to_buffer(connptr->sbuffer, iconvchar, bytesout) < 0) {
                                TRACE("readbuff: add_to_buffer() error.\n");
                                bytes_received=0;
                                //break;
                            } else {
...
extern int errno;
//by nemo: iconv function
int iconv_string(const iconv_t cd,
                 char *src, size_t len,
                 char* result)
{
    int c = 1;
    ...
    while(inbytesleft > 0 && !done)  {
        ...
        if (dst != poutbuf)  {// we have something to write
            ...
            strncat(result,dst, poutbuf-dst);
        }
...
/*by nemo:
  removechar: remove all target substrings in buffer strings, return 0 indicates success, return -1 means not found.
  warning: len and targetlen must be from strlen().
*/
int removechar(char* buffer, size_t len, char* target, size_t targetlen) {
    char* pointstart;
    char* pointend;
    int bufferptr=0;
    int i;
    char swap[READ_BUFFER_SIZE];
    strcpy(swap,buffer);
    pointstart=swap;

    while(1) {
    pointend=strstr(pointstart,target);
    if (pointend!=NULL) {
        for(i=0;i<pointend-pointstart;i++) {
            *(buffer+bufferptr+i)=*(pointstart+i);
        };
        bufferptr+=i;
    } else {
        pointend=swap+len+1; //include '\0'
        for(i=0;i<pointend-pointstart;i++) {
            *(buffer+bufferptr+i)=*(pointstart+i);
        };
        break;
    }
    pointstart=pointend+targetlen;
    }
    if (pointstart==swap) {
        return -1;
    } else {
