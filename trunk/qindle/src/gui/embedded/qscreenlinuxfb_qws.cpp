...
Qeink::Qeink()
{
    ...
    connect(timer, SIGNAL(timeout()), this, SLOT(einkupdate()));
    reflashrect = new QRect;
    einkfd=open("/proc/eink_fb/update_display",O_WRONLY);
    //einkfd=open("/tmp/root/update_display",O_WRONLY);
    if (einkfd<0) {
        qFatal("Error opening eink hal");
    }
...
void Qeink::fbupdate(const QRect & rect)
{
    if (reflashrect->isEmpty()) {
        *reflashrect=rect;
        haschanged=false;
        timer->start(EINK_UPDATE_INTERIAL);
    } else {
        *reflashrect=reflashrect->united(rect);
        haschanged=true;
    }
}

void Qeink::einkupdate()
{
    if (haschanged) {
        haschanged=false;
        timer->start(EINK_UPDATE_INTERIAL);
