...
signals:
    void sendKeyboardData(input_event event);
private slots:
    void getKeyEvent();
    void getMouseEvent();
    void readKeyboardData(input_event event);
...
QWSUsbKbPrivate::QWSUsbKbPrivate(QWSPC101KeyboardHandler *h, const QString &device) : handler(h)
{
    ...
    QSocketNotifier *notifierkey, *notifiermouse;
    if (fd >= 0) {
        notifierkey = new QSocketNotifier(fd, QSocketNotifier::Read, this);
        connect(notifierkey, SIGNAL(activated(int)),this,
                 SLOT(getKeyEvent()));
    }
    fd2 = ::open("/dev/input/event1",O_RDONLY, 0);
    if (fd2 >= 0) {
        notifiermouse = new QSocketNotifier(fd2, QSocketNotifier::Read, this);
        connect(notifiermouse, SIGNAL(activated(int)),this,
                 SLOT(getMouseEvent()));
    connect(this,SIGNAL(sendKeyboardData(input_event)),this,SLOT(readKeyboardData(input_event)));
    }
}
...
void QWSUsbKbPrivate::getKeyEvent()
{
    input_event event;
    ...
    if (event.type != EV_KEY)
        return;
    emit sendKeyboardData(event);
}

void QWSUsbKbPrivate::getMouseEvent()
{
    input_event event;
    if (read(fd2, &event, sizeof(input_event)) != sizeof(input_event))
        return;
    if (event.type != EV_KEY)
        return;
    emit sendKeyboardData(event);
